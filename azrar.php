...
$_az_cache = __DIR__ . '/.bot_cache.json';
if (!isset($userBot) || empty($userBot)) {
    if (file_exists($_az_cache)) {
        $_az = json_decode(file_get_contents($_az_cache));
        $userBot = $_az->username ?? '';
        $bot_id  = $_az->id ?? 'default_bot';
    }
    if (empty($userBot)) {
        $bot_info = bot("getMe");
        $userBot  = $bot_info->result->username ?? '';
        $bot_id   = $bot_info->result->id ?? 'default_bot';
        file_put_contents($_az_cache, json_encode(['id' => $bot_id, 'username' => $userBot]));
    }
}
...
$reply_markup[] = [['text' => 'رجوع', 'callback_data' => '@S_P_P1']];
...
$asia_code = "`SALEH:" . base64_encode('asia_charge_menu') . "`";
bot("sendmessage",[
'chat_id' => $chat_id,
'text' => "".$tm."
...
bot('sendMessage', [
'chat_id' => $chat_id,
'text' => "⚠️ *الكود فارغ!*\n\nكود `SALEH:` بدون قيمة لا يمكن استخدامه.\n\n📌 *أمثلة على الأكواد المتاحة:*\n🖥 طلب الخدمات من الموقع:\n`SALEH:c2VydmljZV93ZWJzaXRl`\n\n📱 شحن اسياسيل:\n`SALEH:" . base64_encode('asia_charge_menu') . "`\n\n💡 _أرسل_ `مشاهدة الازرار` _رداً على أي رسالة تحتوي أزراراً لمعرفة أكوادها_",
'parse_mode' => 'MarkDown',
'reply_markup' => json_encode(['inline_keyboard' => [[['text' => '• رجوع •', 'callback_data' => 'zrar']]]])
]);
exit;
}
if ($callback === 'service_website') {
$SALEh['SALEhs'][$code] = ['name' => $name, 'mo' => '__SERVICE_URL__', 'Type' => 'web_app'];
$replyText = "*• تم حفظ الزر (موقع الخدمات - web_app)*";
} else {
...
$reply_p[] = [['text'=>"رجوع",'callback_data'=>"NAMERO"]];
