<?php
// azrar.php - المنطق الأساسي للبوت (بدون تعريف bot)
...
$file = $NAMERO . 'SALEh.json';

$SALEh = file_exists($file) ? json_decode(file_get_contents($file), true) : [];

function save($array){
    global $file;
    file_put_contents($file, json_encode($array, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
}
...
if ($data == "zrar") {
    ...
    $reply_markup[] = [['text' => 'رجوع', 'callback_data' => '@ypui5']];
    ...
    file_put_contents($NAMERO . "set.txt", ".");
    $SALEh['n'] = null;
    $SALEh['mode'] = null;
    save($SALEh);
    exit;
...
if (preg_match("/^addbtn(\\|(.+))?$/", $data, $m)) {
    ...
    bot('EditMessageText', [
        ...
        'parse_mode' => 'markdown',
        'reply_markup' => json_encode([ 'inline_keyboard' => [ [['text' => '• رجوع •', 'callback_data' => 'zrar']] ] ])
    ]);
    save($SALEh);
    exit;
}
...
if ($text != '/start' && $SALEh['mode'] == 'addm') {
    ...
    if (preg_match("#^https?://#", $text)) {
        $SALEh['links'][$code] = [ 'name' => $name, 'url' => $text ];
        $SALEh['rows'][$row][] = $code;
        $replyText = "*• تم حفظ الزر (رابط)*";
    } elseif (preg_match("#^SALEH:#", $text)) {
        $callback = base64_decode(str_replace("SALEH:", "", $text));
        $SALEh['SALEhs'][$code] = [ 'name' => $name, 'mo' => $callback, 'Type' => 'callback' ];
        $SALEh['rows'][$row][] = $code;
        $replyText = "*• تم حفظ الزر (كود كول باك)*";
    } else {
        $SALEh['SALEhs'][$code] = [ 'name' => $name, 'mo' => $text, 'Type' => 'EditMessageText' ];
        $SALEh['rows'][$row][] = $code;
...
if ($zhend[0] == "zh") {
    ...
    if (isset($SALEh['SALEhs'][$id])) {
        ...
        if ($type == "callback") {
            $fro = "كود كول باك";
            $buttons = [ [['text' => "مسح الزر ️", 'callback_data' => "delete|$id"]], [['text' => "رجوع 🔙", 'callback_data' => "zrar"]] ];
        } else {
            $fro = "محتوى نصي";
            $show = [ "EditMessageText" => "تعديل الرسالة ", "sendMessage" => "إرسال الرسالة ", "sendMessageSilent" => "همسة " ][$type] ?? "تعديل الرسالة ";
            $buttons = [ [['text' => "طريقة عرض النص: $show", 'callback_data' => "showtype:$id"]], [['text' => "مسح الزر ", 'callback_data' => "delete|$id"]], [['text' => "رجوع 🔙", 'callback_data' => "zrar"]] ];
        }
        bot('editMessageText', [
            'chat_id' => $chat_id,
            'message_id' => $message_id,
    ...
    if (isset($SALEh['links'][$id])) {
        ...
        $buttons = [ [['text' => "تعديل الرابط ", 'callback_data' => "editlink|$id"]], [['text' => "مسح الزر ", 'callback_data' => "delete|$id"]], [['text' => "رجوع 🔙", 'callback_data' => "zrar"]] ];
        bot('editMessageText', [
            'chat_id' => $chat_id,
            'message_id' => $message_id,
            'text' => "*• اسم الزر:* $name\n\n*• نوع الزر:* رابط خارجي\n\n`$url`",
...
if (preg_match("#^showtype:(.+)$#", $data, $m)) {
    ...
    $options = [ ['EditMessageText', 'تعديل الرسالة '], ['sendMessage', 'إرسال الرسالة '], ['sendMessageSilent', 'همسة '] ];
    $markup = [];
...
if (preg_match("#^settype:(EditMessageText|sendMessage|sendMessageSilent):(.+)$#", $data, $m)) {
    ...
    save($SALEh);
    $show = [ "EditMessageText" => "تعديل الرسالة ", "sendMessage" => "إرسال الرسالة ", "sendMessageSilent" => "همسة " ][$new];
    bot('answerCallbackQuery', [ 'callback_query_id' => $update->callback_query->id, 'text' => "تم التغيير إلى: $show", 'show_alert' => false ]);
    bot('editMessageText', [
        'chat_id' => $chat_id,
        'message_id' => $message_id,
...
$price = str_replace('#username', "[$user]", $price);
